...
public class BookServiceImpl implements BookService {
    ...
    private final static String BOOKS_FILE_PATH = "D:\\Spring\\ADVQuering\\src\\main\\resources\\files\\books.txt";
    ...
    @Override
    public List<String> getAllBookTitles(AgeRestriction ageRestriction) {
    ...
    @Override
    public List<String> getAllBookTitlesWithLessThanCopiesAndGoldenEdition() {

        return this.bookRepository.findAllByCopiesIsLessThanAndEditionType(5000, EditionType.GOLD)
                .stream()
                .map(book -> String.format("%s", book.getTitle()))
                .collect(Collectors.toList());
    }
    ...
    @Override
    public List<String> priceBook() {
        return this.bookRepository.priceFind(new BigDecimal("5.00"), new BigDecimal("40"));

    }

    @Override
    public List<String> getBookTitleByAuthorLastName(String pattern) {
        return this.bookRepository.findAllBooksByAuthor(pattern)
                .stream()
    ...
    @Override
    public List<String> authorCopies() {
        return this.bookRepository.authorCopies();
    }
    ...
    public List<String> findAllBooksByTitle(String title) {
        return this.bookRepository.findAllByTitle(title)
                .stream()
                .map(book -> String.format("%s %s %s %s", book.getTitle(), book.getEditionType(),
                        book.getAgeRestriction(), book.getPrice()))
                .collect(Collectors.toList());
    }

    @Override
    public void updateBookCopies(int copies, LocalDate date) {
        this.bookRepository.updateCopies(copies, date);
    }
